...
local function calcScore(word, dt)
    local base = #word * 100
    local bonus = 0
    if     dt < 5  then bonus = base * 0.5
    elseif dt < 12 then bonus = base * 0.25
    end
...
local function checkSelection(gameGroup, cat, scoreDisplay, hud)
    ...
    -- Score popup
    local popStr = hasBonus and ("+" .. pts .. " ⚡") or ("+" .. pts)
    local popup = P.text(gameGroup, popStr, W/2, H/2-28, hasBonus and 22 or 17,
                          native.systemFontBold,
                          hasBonus and P.amber or P.moss)
...
local function buildGrid(parent, cat)
    ...
    local gridBg = display.newRect(parent,
        ...
        totalW+6, totalH+6)
    gridBg:setFillColor(P.parchment[1], P.parchment[2], P.parchment[3])
    gridBg.strokeWidth = 1.5
    gridBg:setStrokeColor(cat.color[1],cat.color[2],cat.color[3], 0.28)
    ...
    for r = 1, sz do
        ...
        for c = 1, sz do
            ...
            bg:setFillColor(P.cream[1], P.cream[2], P.cream[3], 0.0)
            bg.strokeWidth = 0.4
            bg:setStrokeColor(P.warmTan[1],P.warmTan[2],P.warmTan[3], 0.45)
            ...
            -- Warm muted letter colour (much easier on eyes than stark green)
            letter:setTextColor(P.bark[1], P.bark[2], P.bark[3])
    ...
    local function resetNonFoundColor(selList)
        for _, pos in ipairs(selList) do
            ...
            if not isFound and state.cells[pr] and state.cells[pr][pc] then
                state.cells[pr][pc].text:setTextColor(P.bark[1],P.bark[2],P.bark[3])
            end
        end
    end
...
function scene:create(event)
    ...
    -- Back button
    local backT = P.text(sg, "✕", 24, 24, 18, native.systemFont, P.bark)
    ...
    P.text(sg, string.upper(difficulty), W/2, 42, 9,
            native.systemFont, cat.color)

    -- Score
    P.text(sg, "SCORE", W-42, 18, 9, native.systemFont, P.bark)
    ...
    local foundTxt = P.text(sg, "0/"..state.totalWords, 42, 52, 14,
                              native.systemFontBold, cat.color)
    state._foundTxt = foundTxt

    -- ── Hint button ────────────────────────────────────
    local hints    = savedata.getHintsAvailable()
    local hintBg   = display.newRoundedRect(sg, W/2, HEADER_H-16, 96, 23, 9)
    hintBg:setFillColor(P.warmTan[1]*0.85, P.warmTan[2]*0.85, P.warmTan[3]*0.85)
    hintBg.strokeWidth = 1
    hintBg:setStrokeColor(P.amber[1],P.amber[2],P.amber[3], 0.55)

    local hintTxt = P.text(sg, "💡 HINT ("..hints..")", W/2, HEADER_H-15, 10,
                            native.systemFontBold, P.amber)
    ...
    wlBg:setFillColor(P.parchment[1],P.parchment[2],P.parchment[3])
    local wlBorder = display.newRect(sg, W/2, wordListY, W, 1)
    wlBorder:setFillColor(P.warmTan[1],P.warmTan[2],P.warmTan[3])

    buildWordList(sg, state.placedWords, cat, wordListY + 6)

    -- ── Touch ──────────────────────────────────────────
    setupTouch(gameGroup, cat, scoreDisplay,
               { foundTxt=foundTxt, hintTxt=hintTxt })
