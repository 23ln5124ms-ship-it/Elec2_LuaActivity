...
local symbols = { "✦", "❋", "✿", "◈", "◆", "❧", "✾" }
local warmCols = {
    P.sage, P.sageLt, P.amber, P.amberLt, P.bark, P.sand, P.sageDk
}
...
function scene:create(event)
    ...
    glow:setFillColor(P.sageLt[1], P.sageLt[2], P.sageLt[3])
    glow.alpha = 0
    ...
    ring1:setStrokeColor(P.sage[1], P.sage[2], P.sage[3], 0.4)
    ring1.alpha = 0

    local ring2 = display.newCircle(sg, W/2, H * 0.42, 125)
    ring2:setFillColor(0,0,0,0)
    ring2.strokeWidth = 0.8
    ring2:setStrokeColor(P.bark[1], P.bark[2], P.bark[3], 0.25)
    ring2.alpha = 0
    ...
    -- Loading bar
    local barW    = W * 0.62
    local barBg   = display.newRoundedRect(sg, W/2, H - 110, barW, 5, 3)
    barBg:setFillColor(P.warmTan[1], P.warmTan[2], P.warmTan[3])
    barBg.alpha = 0

    local barFill = display.newRoundedRect(sg, W/2 - barW/2, H - 110, 2, 5, 3)
    barFill.anchorX = 0
    barFill:setFillColor(P.sageDk[1], P.sageDk[2], P.sageDk[3])
    barFill.alpha = 0

    local loadTxt = P.text(sg, "Planting seeds…", W/2, H - 132, 11,
                            native.systemFont, P.bark)
    loadTxt.alpha = 0

    local tapHint = P.text(sg, "touch anywhere to begin", W/2, H - 58, 12,
                            native.systemFont, P.sage)
    tapHint.alpha = 0
    ...
    self._refs = {
        ...
        loadTxt=loadTxt, tapHint=tapHint,
        particles=particles,
    }
end
...
function scene:show(event)
    ...
    timer.performWithDelay(4200, function()
        ...
        transition.to(r.tapHint, { time=400, alpha=1.0, onComplete = function()
            local function pulse()
                transition.to(r.tapHint, { time=900, alpha=0.3, onComplete=function()
                    transition.to(r.tapHint, { time=900, alpha=1.0, onComplete=pulse })
                end })
            end
            pulse()
        end})
